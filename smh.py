...
st.text("")
st.markdown("""---""")
st.subheader('In this step I check if my dataset has some NaN')
st.metric(label='NaN', value=df.isnull().sum().sum())
st.markdown('As we can see, my dataset has some NaN')
...
st.text("")
st.text("")
st.subheader('Now I check if my dataset has some NaN again')
st.metric(label='NaN', value=df.isnull().sum().sum())
print(df.isnull().sum().sum())
...
st.header('Plots:')
st.subheader(
    'Now I make simple plots with different types for some numerical data. In order to put any hypotheses, I want to look at a couple of dependencies.')
st.markdown(
...
print(df['imbd_votes'].corr(df['imdb_rating']))''')
st.markdown('On the graph, you can see a positive dependence, that is, the number of votes strongly affects the high rating. This information can help in making hypotheses')
...
st.subheader('The first hypothesis: it is believed that in 2005-2010, drama were rated higher than action. And I want to check this hypothesis')
...
plt.legend()
plt.show()''')
st.markdown('As the graph shows, for the period 2005-2010 action  were the most highly rated. This means that the hypothesis was invalid, and we proved it!')

st.markdown("""---""")
st.subheader('The second hypothesis: it is believed that since 2000, crime films have received more votes than mystery and fantasy, that is, they are in greater demand among the rest. For proof this, I chose three years, when there were coincidences in releases (2003, 2006 and 2021), for which I will compare.')
st.markdown('''Firstly, let's find the average number of votes among crime for the given three years''')
...
st.text("")
st.text("")
st.markdown('Next, we find the average number of votes among mystery for the given three years')
...
st.text("")
st.text("")
st.markdown('Then we find the average number of votes among fantasy for the given three years')
